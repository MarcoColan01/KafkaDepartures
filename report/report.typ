...
#show: it => basic-report(
  ...
  doc-title: "KafkaDepartures: a Real-Time Cloud-Native departure dashboard for three European airports (Amsterdam-Schiphol, Helsinki-Vantaa & Oslo-Gardermoen)",
...
- *Schiphol, Finavia and Avinor Pollers:* three host-side Python adapters, one per airport. Each polls its provider every 30 seconds, maintains a sliding window of 20 imminent flights, translates the airport-specific API format (paginated JSON for Schiphol, namespaced XML for Helsinki and attribute-based XML for Oslo) into a canonical `FlightEvent` schema, and forwards events via HTTP POSTs to the API-producer.
- *API-producer:* a FastAPI service exposing a `POST /flight` endpoint with Pydantic validation. Publishes events to the `flight.departures` topic with `acks=all`, `enable.idempotence=True`, infinite retries, and gzip compression. A custom `ResilientProducer` wrapper detects the idempotent producer's fatal state - that may occur after prolonged broker unavailability - and transparently reinstantiates it without container restart.
- *Stats-aggregator:* a Kafka Consumer and Producer that builds rolling counters of departed flights and average delays per airport. On every boot, it seeks back to 00:00 UTC and rebuilds the day's state from the log. Snapshots are published on `flight.statistics` every 10 seconds. The delay is computed through a cascading fallback: precomputed `delay_minutes` → `actual_departure` → `estimated_departure` → `observed_at_utc` (the time the poller observed the departure).
- *Notifier:* Kafka Consumer/Producer that emits a human-readable alert on `flight.departed_flights` whenever a flight transitions to DEPARTED, using the same delay cascade. Destination IATA codes are resolved to city names via the `airportsdata` library. An in-memory deduplication set guarantees one alert per flight.
- *Dashboard:* Flask web application with three independent Kafka consumers (one per topic) and a Server-Sent Events stream toward the browser. Renders three independent boards (one per airport), each with departure statistics, a scrollable feed of today's notifications, and the next 20 scheduled flights.
...
- `flight.departures`: 6 partitions, replication factor = 3 and min.insync.replicas = 2
- `flight.statistics`: 3 partitions, same replication settings as telemetry
- `flight.departed_flights`: 3 partitions, same replication settings as telemetry
