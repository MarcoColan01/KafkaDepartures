...
- *Schiphol, Finavia and Avinor Pollers:* three host-side Python adapters, one per aiport. Each polls its provider every 30 seconds, maintains a sliding window of 20 imminent flights, translates the airport-specific API format (paginated JSON for Schiphol, namespaced XML for Helsinki and attribute-based XML for Oslo) into a canonical `FlightEvent` schema, and forwards events via HTTP POSTs to the API-producer.
